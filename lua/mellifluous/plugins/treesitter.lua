return function(highlights, colors)
    ...
    return lush(function(injected_functions)
        ...
        return {
            sym('@attribute')           { fg = colors.other_keywords.hex } , -- Annotations that can be attached to the code to denote some kind of meta information. e.g. C++/Dart attributes.
            ...
            sym('@debug')               { fg = colors.other_keywords.hex } , -- Debugging statements.
            ...
            sym('@field')               { fg = colors.fg.hex, gui = config.styles.properties } , -- Object and struct fields.
            ...
            sym('@keyword.function')    { fg = colors.main_keywords.hex, gui = config.styles.keywords } , -- Keywords used to define a function: `function` in Lua, `def` and `lambda` in Python.
            sym('@keyword.operator')    { fg = colors.other_keywords.hex, gui = config.styles.operators } , -- Unary and binary operators that are English words: `and`, `or` in Python; `sizeof` in C.
            sym('@keyword.return')      { fg = colors.main_keywords.hex, gui = config.styles.keywords } , -- Keywords like `return` and `yield`.
            ...
            sym('@parameter')           { fg = colors.fg.hex } , -- Parameters of a function.
            ...
            sym('@text.title')          { fg = colors.other_keywords.hex } , -- Text that is part of a title.
            sym('@text.literal')        { highlights.Character } , -- Literal or verbatim text.
            sym('@text.uri')            { fg = change_color.get_lower_contrast(colors.blue, 15).hex } , -- URIs like hyperlinks or email addresses.
            sym('@text.todo')           { highlights.Todo },
            sym('@math')                { fg = colors.other_keywords.hex } , -- Math environments like LaTeX's `$ ... $`
            sym('@text.reference')      { fg = colors.constants.hex } , -- Footnotes, text references, citations, etc.
            sym('@environment')         { fg = colors.other_keywords.hex } , -- Text environments of markup languages.
            ...
            sym('@note')                { fg = colors.special_comments.hex } , -- Text representation of an informational note.
            sym('@warning')             { fg = colors.orange.hex } , -- Text representation of a warning note.
            sym('@danger')              { fg = colors.red.hex } , -- Text representation of a danger note.
            ...
            sym('@variable.builtin')    { fg = colors.other_keywords.hex } , -- Variable names defined by the language: `this` or `self` in Javascript.
        }
    end)
end
